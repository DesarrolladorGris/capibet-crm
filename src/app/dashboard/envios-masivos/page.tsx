  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="bg-[#1a1d23] border-b border-[#3a3d45] px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-white font-semibold text-2xl">Envíos masivos</h1>
          </div>
      ...
      <div className="flex-1 bg-[#1a1d23] p-6">
        ...
        {activeTab === 'nuevo' && (
          <div>
            <h2 className="text-white font-semibold text-xl mb-6">Nuevo envío masivo</h2>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                {/* Buscar sesión */}
                <div>
                  <label className="block text-gray-400 text-sm mb-2">Buscar sesión</label>
                  <select className="w-full bg-[#2a2d35] border border-[#3a3d45] rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#00b894] focus:border-[#00b894]">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-gray-400 text-sm">Ingresa números de teléfono separados por comas</label>
                    <div className="flex items-center space-x-2">
                      ...
                      <button className="bg-[#2a2d35] text-white px-3 py-1 rounded text-sm hover:bg-[#3a3d45] transition-colors">
                        CSV
                      </button>
                    </div>
                  </div>
                  <textarea
                    ...
                    className="w-full bg-[#2a2d35] border border-[#3a3d45] rounded px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#00b894] focus:border-[#00b894] resize-none"
                    rows={4}
                ...
                {/* Agregar otra sesión */}
                <button className="text-[#00b894] hover:text-[#00a085] text-sm font-medium transition-colors">
                  + AGREGAR OTRA SESIÓN
                </button>
                ...
                <div className="bg-[#2a2d35] rounded-lg border border-[#3a3d45] p-4">
                  <div className="flex items-center space-x-3 mb-3">
                    <div className="w-8 h-8 bg-[#1a1d23] rounded-full flex items-center justify-center">
                      <span className="text-gray-400">😊</span>
                    </div>
                    <button className="w-6 h-6 bg-[#00b894] rounded-full flex items-center justify-center text-white text-sm">
                      +
                    </button>
                  ...
                  <button className="text-[#00b894] hover:text-[#00a085] text-sm font-medium mt-2 transition-colors">
              ...
              <div className="space-y-6">
                {/* Buscar contacto */}
                <div>
                  <label className="block text-gray-400 text-sm mb-2">Buscar contacto</label>
                  <select className="w-full bg-[#2a2d35] border border-[#3a3d45] rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#00b894] focus:border-[#00b894]">
                ...
                <div className="bg-[#2a2d35] rounded-lg border border-[#3a3d45] p-4">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <span className="text-white font-medium">Mensajes por ronda y sesión</span>
                  </div>
                  
                  <p className="text-gray-400 text-sm mb-4">Mensajes enviados por sesión antes del standby</p>
                  
                  <input
                    type="number"
                    value={messagesPerRound}
                    onChange={(e) => setMessagesPerRound(parseInt(e.target.value) || 10)}
                    className="w-full bg-[#1a1d23] border border-[#3a3d45] rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#00b894] focus:border-[#00b894] mb-4"
                  />

                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-white font-medium mb-1">Tiempo de espera (segundos)</div>
                      <div className="text-gray-400 text-sm">Tiempo de espera tras enviar los mensajes</div>
                    </div>
                  ...
                  {enableWaitTime && (
                    <input
                      ...
                      className="w-full bg-[#1a1d23] border border-[#3a3d45] rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#00b894] focus:border-[#00b894] mt-3"
                    />
                  )}
                </div>
              </div>
            ...
            <div className="mt-8">
              <button
                ...
                className="w-full bg-[#00b894] hover:bg-[#00a085] text-white py-3 rounded-lg font-medium text-lg transition-colors"
              >
        ...
        {activeTab === 'lista' && (
          <div>
            ...
            <div className="bg-[#2a2d35] rounded-lg border border-[#3a3d45]">
              <div className="grid grid-cols-6 gap-4 p-4 border-b border-[#3a3d45] text-gray-400 text-sm font-medium">
                <div>Progreso</div>
              ...
              {bulkMessages.length === 0 ? (
                ...
                <div className="divide-y divide-[#3a3d45]">
                  {bulkMessages.map((bulk) => (
                    <div key={bulk.id} className="grid grid-cols-6 gap-4 p-4 hover:bg-[#3a3d45] transition-colors">
                      <div className="text-white">
                        <div className="w-full bg-[#1a1d23] rounded-full h-2 mb-1">
                          <div 
                            className="bg-[#00b894] h-2 rounded-full transition-all"
                            style={{ width: `${bulk.progreso}%` }}
                          ></div>
                        </div>
                        <span className="text-xs text-gray-400">{bulk.progreso}%</span>
                      </div>
                      <div className="text-white">{bulk.totalMensajes}</div>
                      <div className="text-green-400">{bulk.entregado}</div>
                      <div className="text-red-400">{bulk.fallido}</div>
                      <div className="text-gray-300">{bulk.creador}</div>
                      <div>
